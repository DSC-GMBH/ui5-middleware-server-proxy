	...
	if(config.reWriteHead){
		proxy.on('proxyReq', function (proxyReq, req, res, options) {
			...
			res.writeHead = function (statusCode, headers) {
				...
				// if(currentSapClient){
				// 	if (!cookie) {
				// 		res.setHeader('set-cookie', ['sap-usercontext=sap-client='+currentSapClient+'; path=/']);
				// 	} else {
				// 		cookie.push('sap-usercontext=sap-client='+currentSapClient+'; path=/')
				// 	}
				// }
				for (let key in cookie) {
					...
					if (cookie[key].match(searchString))
						cookie[key] = cookie[key].substring(0, cookie[key].indexOf("domain")) + "domain=localhost";
				}
